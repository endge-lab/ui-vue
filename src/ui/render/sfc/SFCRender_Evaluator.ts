...
import type { SFCVueRenderBinding, SFCVueRenderContext } from '@/model/render/sfc/sfc-vue-render.type'
...
function evaluateLogicalExpression(
  ...
  if (node.operator === '??') {
    return left ?? evaluateExpressionNode(rightNode, context)
  }
...
function evaluateConditionalExpression(
  ...
  if (test === UNSUPPORTED_EXPRESSION) {
    return test
  }
...
function evaluateMemberExpression(
  ...
  if (property === UNSUPPORTED_EXPRESSION) {
    return property
  }
...
function evaluateCallExpression(
  ...
  if (globalName !== UNSUPPORTED_EXPRESSION) {
    ...
    if (globalResult !== UNSUPPORTED_EXPRESSION) {
      return globalResult
    }
  }
...
function evaluateCallArguments(
  ...
  for (const argument of value) {
    ...
    if (evaluated === UNSUPPORTED_EXPRESSION) {
      return evaluated
    }
...
function evaluateArrayExpression(
  ...
  for (const element of node.elements) {
    ...
    if (!elementNode || elementNode.type === 'SpreadElement') {
      return UNSUPPORTED_EXPRESSION
    }
