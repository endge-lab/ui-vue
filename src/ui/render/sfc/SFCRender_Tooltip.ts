...
export const SFCRender_Tooltip: SFCVueRenderFunction = (input) => {
  ...
  attachEndgeTooltipTriggerAttrs(attrs, input.context.tooltipManager ?? null, anchor => ({
    ...
    renderContent: () => {
      if (kind === 'text') return String(evaluateProp(input, 'text') ?? '')
      if (kind === 'markdown') return renderMarkdown(input, evaluateProp(input, 'markdown'))
  ...
  if (only && isVNode(only)) return cloneVNode(only, attrs, true)
  return input.h('span', {
    ...attrs,
...
function renderInline(
  ...
  return nodes.map((node, index) => {
    ...
    if (node.kind === 'code') return input.h('code', { key: index }, node.value)
    if (node.kind === 'strong') return input.h('strong', { key: index }, renderInline(input, node.children))
    if (node.kind === 'emphasis') return input.h('em', { key: index }, renderInline(input, node.children))
    return input.h('a', {
      key: index,
      href: node.href,
      tabindex: -1,
...
function evaluateProp(
  ...
  for (const name of names) {
    ...
    if (value) return evaluateSFCValue(value, input.context)
  }
  return undefined
}
