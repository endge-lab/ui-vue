...
import type { EndgeTanStackTableColumn, EndgeTanStackTableProps } from './tanstack-table.types'
...
import { resolveTanStackTableFallback } from './tanstack-table-i18n'
import { TanStackTableRuntimeContextKey } from './tanstack-table-runtime-context'
import TanStackTableColumnManager from './TanStackTableColumnManager.vue'

defineOptions({ name: 'EndgeTanStackSfcDataTable' })

const props = withDefaults(defineProps<EndgeTanStackTableProps>(), {
  columns: () => [],
  source: () => [],
  eventBindings: () => [],
  defaultSort: () => [],
...
const tableColumns = computed<EndgeTanStackTableColumn[]>(() => Array.isArray(props.columns) ? props.columns : [])
...
const pageSizeItems = computed<number[]>(() => Array.isArray(props.pageSizes) ? props.pageSizes : [25, 50, 100])
...
const runtimeContext = inject(TanStackTableRuntimeContextKey, null)
...
const columnSizing = ref<ColumnSizingState>(readTableRecordState('size', {}))
...
const resolvedSelectionTrigger = computed(() => props.selectionTrigger === 'auto' ? 'control' : props.selectionTrigger)
...
const columnDefinitions = computed<ColumnDef<Record<string, unknown>>[]>(() => [
  ...
  ...tableColumns.value.map(column => ({
    id: column.key,
    accessorFn: (row: Record<string, unknown>) => readSFCObjectPath(column.key, row),
    header: column.title,
    ...
    enableSorting: props.sortMode !== 'disabled'
      && props.sortMode !== 'fixed'
...
const table = useVueTable({
  ...
  getRowId: (row, index) => normalizeRowId(readSFCObjectPath(props.rowKey, row), index),
...
const virtualRows = computed<VirtualTableRow[]>(() => {
  const items = props.paging === 'pages'
    ? tableRows.value.map((row, index) => ({
        index,
        key: row.id,
        start: index * props.rowSize,
        end: (index + 1) * props.rowSize,
        size: props.rowSize,
        lane: 0,
      }))
    : virtualItems.value
...
const virtualBodyHeight = computed(() => tableRows.value.length === 0
  ? 96
  : props.paging === 'pages'
    ? tableRows.value.length * props.rowSize
...
function translateTableText(
  ...
  return Endge.i18n.t(key, {
    defaultValue: resolveTanStackTableFallback(key, Endge.i18n.locale, fallback, params),
    params,
  })
}
...
function openColumnMenu(
  ...
  const anchor = anchorKind === 'element' && event.currentTarget instanceof Element
    ? event.currentTarget.getBoundingClientRect()
    : null
  openEndgeContextMenu({
    ownerId: props.boundaryId,
    x: anchor?.left ?? event.clientX,
    y: anchor?.bottom ?? event.clientY,
    menu,
    context,
  })
}
...
function resolveColumnMenu(column: EndgeTanStackTableColumn): ContextMenuDescriptor | null {
  ...
  if (tableColumn.getCanSort()) {
    items.push(
      menuItem(
        TABLE_RUNTIME_ACTION_IDS.sortSetColumnAsc,
        translateTableText(TABLE_RUNTIME_ACTION_IDS.sortSetColumnAsc, 'Sort ascending'),
      ),
      menuItem(
        TABLE_RUNTIME_ACTION_IDS.sortSetColumnDesc,
        translateTableText(TABLE_RUNTIME_ACTION_IDS.sortSetColumnDesc, 'Sort descending'),
      ),
    )
    if (tableColumn.getIsSorted()) {
      items.push(menuItem(
        TABLE_RUNTIME_ACTION_IDS.sortClearColumn,
        translateTableText(TABLE_RUNTIME_ACTION_IDS.sortClearColumn, 'Clear column sort'),
      ))
    }
  }
  if (column.pinnable && tableColumn.getCanPin()) {
    if (items.length) {
      items.push({ kind: 'separator', id: `${column.key}-sort-pin-separator` })
    }
    if (tableColumn.getIsPinned() !== 'left') {
      items.push(menuItem(
        TABLE_RUNTIME_ACTION_IDS.columnPinLeft,
        translateTableText(TABLE_RUNTIME_ACTION_IDS.columnPinLeft, 'Pin left'),
      ))
    }
    if (tableColumn.getIsPinned() !== 'right') {
      items.push(menuItem(
        TABLE_RUNTIME_ACTION_IDS.columnPinRight,
        translateTableText(TABLE_RUNTIME_ACTION_IDS.columnPinRight, 'Pin right'),
      ))
    }
    if (tableColumn.getIsPinned()) {
      items.push(menuItem(
        TABLE_RUNTIME_ACTION_IDS.columnUnpin,
        translateTableText(TABLE_RUNTIME_ACTION_IDS.columnUnpin, 'Unpin'),
      ))
    }
  }
  if (tableColumn.getCanHide()) {
    if (items.length) {
      items.push({ kind: 'separator', id: `${column.key}-visibility-separator` })
    }
    items.push(menuItem(
      TABLE_RUNTIME_ACTION_IDS.columnHide,
      translateTableText(TABLE_RUNTIME_ACTION_IDS.columnHide, 'Hide column'),
    ))
  }
  return items.length ? { kind: 'context-menu', items } : null
}

function createColumnActionContext(
  column: Column<Record<string, unknown>>,
  descriptor: EndgeTanStackTableColumn,
): TableColumnActionContext {
...
function resolvePatchedRowIndex(rows: Record<string, unknown>[], patch: RuntimeCollectionProjectionUpdatePatch): number {
  const key = isPlainObject(patch.itemSnapshot) ? readSFCObjectPath(props.rowKey, patch.itemSnapshot) : undefined
  if (key != null) {
    const index = rows.findIndex(row => Object.is(readSFCObjectPath(props.rowKey, row), key))
    if (index >= 0) {
      return index
    }
  }
...
function getColumnDescriptor(column: Column<Record<string, unknown>>): EndgeTanStackTableColumn {
  return (column.columnDef.meta as any).endgeColumn as EndgeTanStackTableColumn
}
...
function getRowAttrs(row: Record<string, unknown>, rowId: string): Record<string, unknown> {
  return getSFCTableRowAttrs(row, getRowStates(rowId))
}
...
function compareRows(left: Record<string, unknown>, right: Record<string, unknown>, column: EndgeTanStackTableColumn): number {
  if (!column.sort) {
    return 0
  }
...
function menuItem(id: string, label: string) {
  return { kind: 'item' as const, id, label, action: id, icon: id }
}
</script>

<template>
  <div
    class="endge-tanstack-table"
    :data-paging="paging"
    :data-lazy="lazy ? 'true' : undefined"
    :style="{ '--endge-table-row-size': `${rowSize}px` }"
    @keydown="onTableKeydown"
  >
    <div class="endge-tanstack-table__toolbar">
      <div v-if="showRuntimeControls" class="endge-tanstack-table__runtime-controls">
        <input
          v-if="showSearch"
          class="endge-sfc-input endge-tanstack-table__search"
          type="search"
          :value="runtimeContext?.searchValue.value ?? ''"
          :placeholder="translateTableText('table.search.placeholder', 'Search…')"
          :aria-label="translateTableText('table.search.ariaLabel', 'Search table')"
        ...
        <button
          v-if="showFilters"
          type="button"
          class="endge-tanstack-button endge-tanstack-button--outline endge-tanstack-table__filter-toggle"
          :data-active="filtersVisible ? 'true' : undefined"
          :aria-expanded="filtersVisible"
          :aria-label="filtersVisible
            ? translateTableText('table.filters.hide', 'Hide filters')
            : translateTableText('table.filters.show', 'Show filters')"
          :title="filtersVisible
            ? translateTableText('table.filters.hide', 'Hide filters')
            : translateTableText('table.filters.show', 'Show filters')"
          @click="toggleFilters"
        >
      ...
      <div
        class="endge-tanstack-table__toolbar-meta"
        :class="{ 'endge-tanstack-table__toolbar-meta--after-controls': showRuntimeControls }"
      >
        <div class="endge-tanstack-table__summary">
          <span>{{ totalRowCount }}</span>
          <span class="endge-tanstack-table__summary-label">
            {{ translateTableText('table.summary.rows', 'rows') }}
          </span>
        </div>
        <TanStackTableColumnManager
          v-if="tableColumns.length > 1"
          :table="table"
          :translate="translateTableText"
        />
      </div>
    </div>
    ...
    <div
      ...
      class="endge-tanstack-table__runtime-filters"
    >
      <EndgeFilterRenderer :runtime="filterRuntime" />
    </div>

    <div
      ref="scrollRoot"
      v-bind="styleContract.grid.attrs"
      class="endge-tanstack-table__viewport"
    >
      <table
        data-slot="table"
        ...
        class="endge-tanstack-table__table"
        :style="{ width: tableWidth }"
      >
        <thead v-bind="styleContract.header.attrs" class="endge-tanstack-table__header">
          <tr>
            <th
              ...
              class="endge-tanstack-table__head"
              :class="{
                'endge-tanstack-table__head--pinned': header.column.getIsPinned(),
                'endge-tanstack-table__selection-head': isSelectionColumn(header.column),
              }"
            ...
            >
              <input
                ...
                class="endge-tanstack-checkbox endge-tanstack-table__selection-input"
                type="checkbox"
                :checked="allVisibleRowsSelected"
                :indeterminate="someVisibleRowsSelected"
                :aria-label="translateTableText('table.selection.visibleRows', 'Select visible rows')"
                @click.stop
              ...
              <button
                ...
                type="button"
                class="endge-tanstack-table__sort"
                :class="{ 'endge-tanstack-table__sort--enabled': header.column.getIsSorted() }"
              ...
              >
                <span v-bind="getColumnDescriptor(header.column).styleSurfaces.headerContent.attrs" class="endge-tanstack-table__head-title">
                  <FlexRender :render="header.column.columnDef.header" :props="header.getContext()" />
                </span>
                <span v-if="header.column.getCanSort()" class="endge-tanstack-table__sort-icon">
                  ...
                  <span v-if="getSortIndex(header.column.id) != null && sorting.length > 1" class="endge-tanstack-table__sort-index">
              ...
              <div
                ...
                class="endge-tanstack-table__resizer"
                :class="{ 'endge-tanstack-table__resizer--active': header.column.getIsResizing() }"
        ...
        <tbody
          ...
          class="endge-tanstack-table__body"
          :style="{ height: `${virtualBodyHeight}px` }"
        >
          <tr
            v-for="virtualRow in virtualRows"
            :key="String(virtualRow.virtualItem.key)"
            v-bind="getRowAttrs(virtualRow.styledRow, virtualRow.row.id)"
            :data-index="virtualRow.rowIndex"
            class="endge-tanstack-table__row"
            :class="[getRowClass(virtualRow.styledRow), {
              'endge-tanstack-table__row--selected': selectedRowIds.has(virtualRow.row.id),
              'endge-tanstack-table__row--multi-selected': selectionMode === 'multiple' && selectedRowIds.has(virtualRow.row.id),
            }]"
          ...
          >
            <td
              ...
              class="endge-tanstack-table__cell"
              :class="{
                'endge-tanstack-table__cell--pinned': cell.column.getIsPinned(),
                'endge-tanstack-table__selection-cell': isSelectionColumn(cell.column),
                'endge-tanstack-table__cell--selected': isCellSelected(virtualRow.row.id, cell.column.id),
              }"
            ...
            >
              <input
                ...
                class="endge-tanstack-checkbox endge-tanstack-table__selection-input"
                :class="{ 'endge-tanstack-table__selection-input--single': selectionMode === 'single' }"
                ...
                :aria-label="translateTableText('table.selection.row', 'Select row')"
                @click.stop
          ...
          <tr v-if="tableRows.length === 0">
            <td :colspan="visibleColumnCount" class="endge-tanstack-table__empty">
              {{ translateTableText('table.empty', 'No data') }}
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <footer
      v-if="paging === 'pages'"
      class="endge-tanstack-table__pagination"
      :aria-label="translateTableText('table.pagination.ariaLabel', 'Table pagination')"
    >
      <div class="endge-tanstack-table__pagination-controls">
        <label class="endge-tanstack-table__page-size">
          <span>{{ translateTableText('table.pagination.rowsPerPage', 'Rows per page') }}</span>
          <span class="endge-tanstack-table__page-size-select">
            <select
              :value="pagination.pageSize"
              :aria-label="translateTableText('table.pagination.rowsPerPage', 'Rows per page')"
              @change="setPageSize"
            >
        ...
        <span class="endge-tanstack-table__page-label">
          {{ translateTableText('table.pagination.pageOf', 'Page {current} of {total}', {
            current: currentPage,
            total: pageCount,
          }) }}
        </span>

        <div class="endge-tanstack-table__page-buttons">
          <button
            type="button"
            :aria-label="translateTableText('table.pagination.firstPage', 'First page')"
          ...
          <button
            type="button"
            :aria-label="translateTableText('table.pagination.previousPage', 'Previous page')"
            :disabled="!table.getCanPreviousPage()"
          ...
          <button
            type="button"
            :aria-label="translateTableText('table.pagination.nextPage', 'Next page')"
          ...
          <button
            type="button"
            :aria-label="translateTableText('table.pagination.lastPage', 'Last page')"
            :disabled="!table.getCanNextPage()"
