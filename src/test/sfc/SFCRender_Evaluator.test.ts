...
describe('вычислитель render SFC', () => {
  ...
  it('передаёт `$data.metaOf` в host как ссылку, не как primitive value', () => {
    const readDataMeta = vi.fn().mockReturnValue({ status: 'waiting' })
    const context = createSFCVueRenderContext(
      { rows: [{ id: 7, flightCarrier: 'SU' }] },
      0,
      { id: 'host-1', entityIdentity: 'meta-owner', getArtifact: () => null, readDataMeta } as any,
    )
